...
\`\`\`javascript
const skill = require('./skills/${skill.name}');
...
class ${this.toClassName(skill.name)} {
  constructor(options = {}) {
  ...
  async run(input) {
    ...
    console.log('[${skill.name}] Running with input:', input);
    return { success: true };
  }
}

module.exports = { ${this.toClassName(skill.name)} };
`;
    
    await fs.writeFile(
      path.join(skillDir, 'scripts', `${skill.name}.js`),
      mainScript
    );
